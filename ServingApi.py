...
class Record(BaseModel):
    ...
    BENE_BIRTH_DT: str = Field(..., description="Date of birth in YYYY-MM-DD format.")
    BENE_DEATH_DT: Optional[str] = Field(None, description="Date of death in YYYY-MM-DD format. Can be null if the beneficiary is alive.")
    ...
    BENE_ESRD_IND: str = Field(..., description="End stage renal disease Indicator. 'Y' for Yes, 'N' for No.", pattern='^[01]$')
    ...
    CLM_FROM_DT: str = Field(..., description="Claims start date in YYYY-MM-DD format.")
    CLM_THRU_DT: str = Field(..., description="Claims end date in YYYY-MM-DD format.")
...
@app.post("/predict", tags=["Prediction"], summary="Predict Admission", description="Predict whether a patient will be admitted based on medical records.")
