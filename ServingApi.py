...
async def predict(record: Record):
    try:
        ...
        record_df.to_csv("data/records.csv")
        try:
        ...
        admission_record.to_csv('data/adm_test_record.csv', index=False)
        admission_prediction = admission_model.predict(admission_record)
        print(admission_prediction)

        if int(admission_prediction[0]) == 1:
            insert_position = processed_record.columns.get_loc('PPPYMT_CAR') + 1
            processed_record.insert(loc=insert_position, column='isAdm', value=1)
            processed_record.drop(columns=['ADMTNG_ICD9_DGNS_CD_vec_2','ADMTNG_ICD9_DGNS_CD_vec_1'],inplace=True)
